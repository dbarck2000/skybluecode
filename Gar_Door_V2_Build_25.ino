...
//
// Open and Close the bifold garage doors using a linear actuator attached to each door
// The open or close operation is triggered via a momentary button, or a remote control
// For each door, check the appropriate limit switch while the door is opening or closing. Stop the door when the limit switch is triggered.
// Stop both doors if:
//    - the button (or remote button) is pressed during door operation
//    - the IR obstruction sensor is triggered and the door is closing
...
const int g_startRampDownMillis = 10000;       // elapsed opening or closing time before starting to ramp the motor speed down;
const int g_rampSpeedChange = 1;               // number of steps to increment or decrement the motor speed when ramping up or ramping down
const int g_startSpeed = 150;                  // initial motor speed when opening or closing (0 to 255)
...
const int g_d1OpenStopDelayMillis = 0;         // milliseconds to delay after the open limit switch is triggered before stopping the motor
const int g_d1ClosedStopDelayMillis = 0;       // milliseconds to delay after the closed limit switch is triggered before stopping the motor
const int g_d2OpenStopDelayMillis = 0;         // milliseconds to delay after the open limit switch is triggered before stopping the motor
const int g_d2ClosedStopDelayMillis = 0;       // milliseconds to delay after the closed limit switch is triggered before stopping the motor

// enum types
enum motorStatus {MOTOR_EXTENDING, MOTOR_RETRACTING, MOTOR_STOPPED}; // EXTENING - opens the door, RETRACTING - closes the door
...
enum doorStopCause {DOOR_STOPPED_LIMIT_SW, DOOR_STOPPED_TIMEOUT, DOOR_STOPPED_OBSTRUCTION, DOOR_STOPPED_BUTTON_WHILE_OPENING, 
  DOOR_STOPPED_BUTTON_WHILE_CLOSING, DOOR_STOPPED_INIT, DOOR_STOPPED_MOTOR_ZERO_CURRENT, DOOR_STOPPED_MOTOR_EXCESS_CURRENT, DOOR_STOPPED_REVERSED_MOTOR_EXCESS_CURRENT, DOOR_STOPPED_NA};
enum doorMovingLEDMode {DM_ON, DM_OFF};
enum rampMode {RAMP_UP, RAMP_DOWN, RAMP_NONE};

// Global Pin assignments
// motor dirver board pins //////////////////////////
// motor 1
...
const byte g_m1In1Pin = 44;   // logic control of motor OUT 1
const byte g_m1In2Pin = 45;   // logic control of motor OUT 2
// motor 2
const byte g_m2FBPin = A10;    // same as for motor 1    
const byte g_m2SFPin = 50;      
const byte g_m2PWMD2Pin = 12;  
const byte g_m2In1Pin = 48;   
const byte g_m2In2Pin = 49;   
// this pin applies to both motors
const byte g_enPin = 39;      // Enable input: when EN is LOW, the both motor driver ICs are in a low-current sleep mode.
// end motor driver board pins
...
// LEDs
const byte g_excessCurrentFaultLEDPin = 3 ;  // light if doorStopCause is DOOR_STOPPED_MOTOR_EXCESS_CURRENT
...
class Motor {
  public:
    Motor(byte fBPin, byte sFPin, byte pWMD2Pin, byte in1Pin, byte in2Pin, byte g_enPin);
  ...
  private:
    ...
    byte mIn1Pin;   
    byte mIn2Pin;       
    byte mEnPin;  // this pin applys to both motors, the same pin is used for both motors

    motorStatus currStatus = MOTOR_STOPPED;
    const unsigned int maxSpeed = 255;
    unsigned int currSpeed = 0;
};
// end Motor class prototype

//
// Motor class implimentation
//
Motor::Motor(byte fBPin, byte sFPin, byte pWMD2Pin, byte in1Pin, byte in2Pin, byte g_enPin) {
  mFBPin = fBPin;
  mSFPin = sFPin;
  mPWMD2Pin = pWMD2Pin;
  mIn1Pin = in1Pin;
...
// extend the linear actuator
//
void Motor::extend(unsigned int speed){
  setSpeed(speed);
  digitalWrite(mIn1Pin,HIGH);
  digitalWrite(mIn2Pin,LOW);
  currStatus = MOTOR_EXTENDING;
}
...
// retrct the linear actuator
//
void Motor::retract(unsigned int speed){
  setSpeed(speed);
  digitalWrite(mIn1Pin,LOW);
  digitalWrite(mIn2Pin,HIGH);
  currStatus = MOTOR_RETRACTING;
}
...
void Motor::stop(){
  ...
  digitalWrite(mIn1Pin,LOW);
  digitalWrite(mIn2Pin,LOW);
  currStatus = MOTOR_STOPPED;
}

//
// rampUp
//
rampMode Motor::rampUp() {
...
//
// getCurrent
// get the milliamps geing drawn by the motor
//
unsigned int Motor::getCurrent() {
...
class Door {
  public:
    Door(Motor* motorPtr, byte openLimitSwPin, byte approachingOpenPin, byte closedLimitSwPin, byte approachingClosedPin, int openStopDelayMillis, int closedStopDelayMillis);
    void init();
    void open();
    void close();
...
// Door() 
//
Door::Door(Motor* motorPtr, byte openLimitSwPin, byte approachingOpenPin, byte closedLimitSwPin, byte approachingClosedPin, int openStopDelayMillis, int closedStopDelayMillis) {
  dMotorPtr = motorPtr;
...
void Door::close() {
  ...
  doorRampMode = RAMP_UP;
  limitSwitchTriggered = false;
  dMotorPtr->retract(g_startSpeed);
}
...
void Door::processEvents() {
  if (currStatus == DOOR_OPENING || currStatus == DOOR_CLOSING) {
    unsigned int motorCurrent = dMotorPtr->getCurrent();
    // Check for over max current
    if (motorCurrent > g_maxCurrent) {
      if (isOverMaxCurrent) {
    ...
    if (overMaxCurrentTotalMillis > g_maxMillisAtOverCurrent) {
      ...
      DEBUG_PRINT(overMaxCurrentTotalMillis);
      stop(DOOR_STOPPED, DOOR_STOPPED_MOTOR_EXCESS_CURRENT);
      return;
    }
    ...
    if (currStatus == DOOR_OPENING) {
      ...
      if (underMinCurrentTotalMillis > g_maxMillisAtUnderCurrent) {
        ...
        DEBUG_PRINT(underMinCurrentTotalMillis);
        stop(DOOR_OPEN, DOOR_STOPPED_MOTOR_ZERO_CURRENT);
        return;
      }
    ...
    if (currStatus == DOOR_CLOSING) {
      ...
      if (underMinCurrentTotalMillis > g_maxMillisAtUnderCurrent) {
        DEBUG_PRINT(" underMinCurrentTotalMillis = ");
        DEBUG_PRINT(underMinCurrentTotalMillis);
        stop(DOOR_CLOSED, DOOR_STOPPED_MOTOR_ZERO_CURRENT);
        return;
      }
    ...
    // ramp motor speed up or down
    if (doorRampMode == RAMP_UP) {
      doorRampMode = dMotorPtr->rampUp();
    } else if(doorRampMode == RAMP_DOWN) {
      doorRampMode = dMotorPtr->rampDown();
  ...
  if (currStatus == DOOR_STOPPED) {
    switch (currStopCause) {
      ...
      case DOOR_STOPPED_OBSTRUCTION:
        return true;
      case DOOR_STOPPED_MOTOR_EXCESS_CURRENT:
        return true;
    }
  }
  return false;  
  ...
  if (currStatus == DOOR_STOPPED) {
    switch (currStopCause) {
      ...
      case DOOR_STOPPED_OBSTRUCTION:
        ...
        return;
      case DOOR_STOPPED_MOTOR_EXCESS_CURRENT:
        DEBUG_PPRINTLN("DOOR_STOPPED_MOTOR_EXCESS_CURRENT");
...
void Door::debugPrintStopCause(doorStopCause stopCause) {
  switch (stopCause) {
    ...
    case DOOR_STOPPED_INIT:
      ...
      break;
    case DOOR_STOPPED_MOTOR_ZERO_CURRENT:
      DEBUG_PRINT("DOOR_STOPPED_MOTOR_ZERO_CURRENT");
      break;
    case DOOR_STOPPED_MOTOR_EXCESS_CURRENT:
      DEBUG_PRINT("DOOR_STOPPED_MOTOR_EXCESS_CURRENT");
      break;
...
void DoorPair::processEvents() {
  ...
  if (doorsAreStopped) {
    digitalWrite(g_onOffObstructionSensorPin, LOW); // turn off the obstruction sensor
    setDoorMovingLEDMode(DM_OFF);
    if (door1Ptr->getCurrStopCause() == DOOR_STOPPED_MOTOR_EXCESS_CURRENT || door2Ptr->getCurrStopCause() == DOOR_STOPPED_MOTOR_EXCESS_CURRENT) {
      ...
      if (!reverseProcessed) {
        ...
        if (getPrevStatus() == DOOR_OPENING) {
          door1Ptr->dMotorPtr->retract();
          door2Ptr->dMotorPtr->retract();
          delay(250);
          door1Ptr->dMotorPtr->stop();        
          door2Ptr->dMotorPtr->stop();
          reverseProcessed = true;
        } else if (getPrevStatus() == DOOR_CLOSING) {
          door1Ptr->dMotorPtr->extend();
          door2Ptr->dMotorPtr->extend();
          delay(250);
          door1Ptr->dMotorPtr->stop();
          door2Ptr->dMotorPtr->stop();
          reverseProcessed = true;
...
void setup()
{
  ...
  motor1Ptr = new Motor(g_m1FBPin, g_m1SFPin, g_m1PWMD2Pin, g_m1In1Pin, g_m1In2Pin, g_enPin);
  door1Ptr = new Door(motor1Ptr, g_d1OpenLimitSwPin, g_d1ApproachingOpenPin, g_d1ClosedLimitSwPin, g_d1ApproachingClosedPin, g_d1OpenStopDelayMillis, g_d1ClosedStopDelayMillis);
  
  // create door 2 object
  motor2Ptr = new Motor(g_m2FBPin, g_m2SFPin, g_m2PWMD2Pin, g_m2In1Pin, g_m2In2Pin, g_enPin);
  door2Ptr = new Door(motor2Ptr, g_d2OpenLimitSwPin, g_d2ApproachingOpenPin, g_d2ClosedLimitSwPin, g_d2ApproachingClosedPin, g_d1OpenStopDelayMillis, g_d1ClosedStopDelayMillis);

  // create and initialize door pair object
  dpPtr = new DoorPair(door1Ptr, door2Ptr);
  dpPtr->init();
  ...
  // IR Sensor / Obstruction Sensor - wired "Normally Closed" to ground - so the arduino pin is pulled low, when the limit switch is triggered it 
  // opens the circuit and the arduino pin will go high
...
void loop()
{
  if (obstructionSensorPtr->triggered(millis()) && dpPtr->getCurrStatus() == DOOR_CLOSING) { 
    // only react to the obstruction sensor if the door is closing
    dpPtr->stop(DOOR_STOPPED, DOOR_STOPPED_OBSTRUCTION);
  } else { 
    if (buttonPtr->triggered(millis())) {
      ...
      if (!buttonPressProcessed) {
        ...
        doorStatus currStatus = dpPtr->getCurrStatus();
        DEBUG_PPRINT("dpPtr->getCurrStatus() = ");
        ...
        if (currStatus == DOOR_OPEN) {
          ...
          // stop doors 
          dpPtr->stop(DOOR_STOPPED, DOOR_STOPPED_BUTTON_WHILE_OPENING);
        } else if (currStatus == DOOR_CLOSING) {
          DEBUG_PRINTLN("door is DOOR_CLOSING");
          // stop doors
          dpPtr->stop(DOOR_STOPPED, DOOR_STOPPED_BUTTON_WHILE_CLOSING);
          ...
          if (dpPtr->getPrevStatus() == DOOR_OPENING) {
            dpPtr->close();
          } else {
            if (dpPtr->getD1PrevStatus() == DOOR_OPENING || dpPtr->getD2PrevStatus() == DOOR_OPENING) {
              dpPtr->close();
            } else if (dpPtr->getD1PrevStatus() == DOOR_CLOSING || dpPtr->getD2PrevStatus() == DOOR_CLOSING) {
              dpPtr->open();
            } else {
              dpPtr->open();
            }
