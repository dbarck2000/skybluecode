...
// Global constants 
// Max and Min current / time
const int g_maxCurrentOpening = 1800;          // if current exceeds this value for an extended time, then stop 
const int g_maxCurrentClosing = 3000;          // if current exceeds this value for an extended time, then stop 
...
const unsigned long g_maxRuntimeMillis = 13000; // max run time before timing out
const int g_startRampDownMillis = 13000;       // elapsed opening or closing time before starting to ramp the actuator speed down;
...
class ObstructionSensor {
  public:
    ObstructionSensor(byte obstructionSensorPin, byte onOffObstructionSensorPin, unsigned long obstructionSensorStartupMillis);
    void turnOn();
    void turnOff();
  ...
  private:
    byte obstSensPin;    
    byte onOffObstSensPin;    
    unsigned long obstSensStartupMillis;
    unsigned long obstSensStartTime = 0;
    bool isOn = false;
    DebouncedSwitch* obstSensSwitchPtr; 
};
// end ObstructionSensor class prototype
...
// ObstructionSensor
//
ObstructionSensor::ObstructionSensor(byte obstructionSensorPin, byte onOffObstructionSensorPin, unsigned long obstructionSensorStartupMillis) {
  obstSensPin = obstructionSensorPin;    
  onOffObstSensPin = onOffObstructionSensorPin;    
  obstSensStartupMillis = obstructionSensorStartupMillis;
  obstSensSwitchPtr = new DebouncedSwitch(obstSensPin, HIGH); 
}
...
void ObstructionSensor::turnOn() {
  digitalWrite(onOffObstSensPin, HIGH); // turn on the obstruction sensor
  isOn = true;
  obstSensStartTime = millis();
...
void ObstructionSensor::turnOff() {
  digitalWrite(onOffObstSensPin, LOW); // turn off the obstruction sensor
  isOn = false;
...
bool ObstructionSensor::isReady() {
  if (isOn) {
    if ((millis() - obstSensStartTime) > obstSensStartupMillis) {
      return true;
    }
  }
  return false;
...
// objectDetected
//
bool ObstructionSensor::objectDetected() {
  if (isReady()) {
    return obstSensSwitchPtr->triggered(millis());
  } else {
    return false;
  }
}
...
void Door::processEvents() {
  ...
  if (currStatus == DOOR_OPENING || currStatus == DOOR_CLOSING) {
    ...
    if (currStatus == DOOR_OPENING) {
      maxCurrent = g_maxCurrentOpening;
    } else {
      maxCurrent = g_maxCurrentClosing;
    }
...
class DoorPair {
  public:
    DoorPair(Door* door1Ptr, Door* door2Ptr, ObstructionSensor* obstSensePtr);
    void init();
    void open();
    void close();
    void stop(doorStatus status, doorStopCause stopCause);
  ...
  private:    
    Door* door1Ptr;
    Door* door2Ptr;
    ObstructionSensor* obstructionSensorPtr;
...
// DoorPair
//
DoorPair::DoorPair(Door* d1Ptr, Door* d2Ptr, ObstructionSensor* obstSensePtr) {
  door1Ptr = d1Ptr;
  door2Ptr = d2Ptr;
  obstructionSensorPtr = obstSensePtr;
}
...
void DoorPair::close() {
  clearFaultLEDs();
  obstructionSensorPtr->turnOn(); // turn on the obstruction sensor
...
void DoorPair::stop(doorStatus status, doorStopCause stopCause) {
  ...
  obstructionSensorPtr->turnOff(); // turn off the obstruction sensor
}
...
void DoorPair::processEvents() {
  ...
  if (doorsAreStopped) {
    ...
    if (obstructionSensorPtr->getIsOn()) {
      DEBUG_PRINTLN("DoorPair::processEvents - 1");
      obstructionSensorPtr->turnOff(); // turn off the obstruciton sensor
    }
    setDoorMovingLEDMode(DM_OFF);
...
void setup()
{
  ...
  // photo electric IR sensor wired "Normally Closed" to ground - so the arduino pin is pulled low
  // when the IR sensor relay is triggered it opens the circuit and the arduino pin will go high
  obstructionSensorPtr = new ObstructionSensor(g_obstructionSensorPin, g_onOffObstructionSensorPin, g_obstructionSensorStartupMillis); 
...
void loop()
{
  if (doorPairPtr->getCurrStatus() == DOOR_CLOSING && obstructionSensorPtr->objectDetected()) {
